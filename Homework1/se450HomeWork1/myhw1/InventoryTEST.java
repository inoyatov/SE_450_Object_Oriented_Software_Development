...
public class InventoryTEST {
	...
	public void testCheckOutCheckIn() {
		// Check
		//Try checkOut with a null value
		try { s.checkOut(null); fail(); } catch ( IllegalArgumentException e ) {}
		//Try checkIn with null value
...
//		System.out.println("This is the number out" + s.get(v1).numOut);
//		System.out.println("Adding one in");
		s.checkOut(v1);
		assertEquals(1, s.get(v1).numOut);
		
		//Trying to check out when no more left to checkout
		try{ s.checkOut(v1); fail(); } catch ( IllegalArgumentException e ) {}
		
		s.checkIn(v1);
		// Try to check in more than what we have in inventory
	...
	public void testGet() {
		...
		// Complete
		s.addNumOwned(v1,  1); 
//		System.out.println(s.get(v1).hashCode());
//		System.out.println(s.get(v1).hashCode());
		assertTrue(s.get(v1).hashCode() != s.get(v1).hashCode());
	...
	public void testToCollection() {
		...
		s.addNumOwned(v1,  1); 
		s.addNumOwned(v2,  1); 
//		System.out.println("This is the number of records in s.toCollections:\n" 
//							+s.toCollection().size());
//		System.out.println("This is the number of records in s:\n"
//				+s.size() );
	    s.toCollection().clear();
	    s.toCollection().remove(v1);
		assertTrue(s.size() == 2);
		s.clear();
		assertTrue(s.size() == 0);
